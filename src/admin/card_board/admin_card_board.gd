...
func _new_spot():
	...
	_spot = spot
	spot.global_position = Vector2(136, 24)
	spot_container.add_child(spot)

func _remove_spot() -> void:
	var spot_object_list := $spot_container.get_children()
	if spot_object_list.size() > 1:
		var last_spot = spot_object_list[spot_object_list.size() - 2]
		_spot_list.pop_back()
