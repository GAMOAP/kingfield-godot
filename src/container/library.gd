...
func close() -> void:
	...
	EventManager.unit_clicked.disconnect(_on_unit_clicked)
...
func select_card(card_id) -> void:
	for card in _cards:
		if card.get_id() == card_id:
			...
			if _unit_selected_data[str(card_id.to_int() / 100)] != card_id:
				DataManager.update_user_team(Global.unit_selected.name, card_id)
